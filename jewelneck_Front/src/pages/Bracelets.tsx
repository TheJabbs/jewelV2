...
import { braceletCollection } from '@/lib/bracelet-data';
...
const Bracelets = () => {
  ...
  const [filteredItems, setFilteredItems] = useState(braceletCollection);
  const [selectedJewelry, setSelectedJewelry] = useState<JewelryItem | null>(null);
  
  // Find min and max prices
  const prices = braceletCollection.map(item => parsePriceToNumber(item.price));
  ...
  useEffect(() => {
    const filtered = filterByPriceRange(braceletCollection, currentMinPrice, currentMaxPrice);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow pt-24 pb-16">
        <div className="container mx-auto px-4">
          <h1 className="text-3xl md:text-4xl font-display font-semibold text-center mb-8">Bracelets Collection</h1>
            ...
            <div className="w-full md:w-3/4">
              {filteredItems.length === 0 ? (
                <div className="text-center p-8">
                  <p>No bracelets found in the selected price range.</p>
                </div>
              ) : (
